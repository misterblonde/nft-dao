...
contract MyGovernorHelper {
    ...
    function transferOwnershipTimelock(address boxAddress, address newTimelock) external {
        require(msg.sender == myGovernor, "Gov Helper: Only Governor can set up sub DAOs.");
        // ownly owner can perform this action, owner is the governorhelper
        BoxLocal(payable(boxAddress)).transferOwnership(newTimelock);
    }
